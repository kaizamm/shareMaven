...
def call(body) {
  ...
  for (int i = 0;i<hostsArry.size();i++) {
    ...
    if (hostsArryLenth.size() == 3) {
      ...
      sh (script: "docker -H"+" "+appIp+":2375 run -d --restart=always --name="+containerName+" "+"-e SERVICE_NAME="+SERVICE_NAME+" "+"-e etcdClusterIp="+etcdClusterIp+" "+"-e appCfgs="+appCfgs.replace("null","").trim()+" "+"-e appTargetName="+appTargetName+" "+"-e instanceId="+instanceId+" "+"-e jmxIp="+appIp+" "+"-e jmxPort="+jmxPort+" "+"-e JAVA_OPTS="+javaOpts.trim()+" " +"-v /opt/logs/"+containerName+":/AppLogs"+" "+"-v /opt/app_persistent/"+containerName+":/opt/app_persistent"+" "+"-p"+" "+jmxPort+":"+jmxPort+" "+"-p"+" "+appExpose+" "+dockerRunOpts.replace("null","").trim()+" "+toImage.trim(),returnStdout: true)
    }
    sleep(10)
    // 获取当前运行容器的状态码
