...
def call(body) {
	...
	for (i = 0; i <config.APP_HOSTS.size(); i++) {
		...
		sh (script: "ssh ${saltmasterIP}  'sudo salt -L "${NgHostName}" cmd.script salt://scripts/nginx_up_down.sh 'down ${NGINX_CONF} ${NGINX_DAEMON} ${APP_HOST} ${APP_PORT}' ' ",returnStdout: true)
